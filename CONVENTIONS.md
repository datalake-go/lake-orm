...
**What this accomplishes.** Navigation is free. A reader looking for "where does the driver talk to Spark?" runs `ls drivers/spark/driver_spark_*` and reads those files, period. A reader who opens the repo in an IDE sees the same answer from the sidebar — the prefix sorts the family into a contiguous block, and the eye lands on it without any command. A new contributor scaffolding a fresh driver family copies the shape. File listings visually cluster related work in alphabetical order because the shared prefix sorts them together.
...
**Grep becomes a navigation tool.** `grep -rn "^func Fetch"` finds every "absence is an error" callsite in one command. `grep -rn "^func Find"` finds every "absence is fine" callsite. `grep -rn "^func Is"` finds every predicate. A reader skimming a call site in the editor gets the same signal without running a command — the verb on the line tells them which variant they're looking at. If the codebase lets `Get` / `Lookup` / `Find` / `Read` all mean "retrieve," none of those greps work — you'd have to grep all four, sort, deduplicate, and then read each one to see which variant it really was. Locking one verb per concept means grep is the answer, not the start of the investigation.
...
**What this accomplishes.** A reader asking "what does a driver have to implement?" runs `cat drivers/drivers.go` and reads the contract, then opens any sibling (`drivers/spark/`, `drivers/duckdb/`) and sees the implementation side-by-side. A reader with the repo open in an IDE sees the same layout in the sidebar at a glance — `drivers/drivers.go` sits directly above the per-driver subfolders, contract and implementations one scroll apart. When the interface and its implementations live in different packages, that conversation needs two `cd` commands and a mental link — and when the interface moves or a new implementation lands, there's a higher chance something falls out of sync. Colocating the interface with its family keeps the two sides in one `grep -r` reach, which is the mechanism that keeps them honest.
...
**Why this matters.** When an error bubbles up through a caller's service and hits their logger as a string, it's often the only signal anyone has about which library produced it. `"no rows"` could come from anywhere. `"lakeorm: no rows"` can be traced to this repo in one grep — and a reader scrolling the log viewer at 3 AM spots the prefix without needing to run that grep at all. Operators who hit a stack trace at 3 AM find the right codebase in seconds; SREs who write log-matching rules can target one library deterministically.
