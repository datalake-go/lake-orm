...
// drivers.Convertible implementation for the Databricks native
// driver. Source closures produced by the helpers in
// driver_databricks_conversions.go return a *sql.Rows; the methods
// below iterate rows.Next / rows.Scan and decode into the caller-
// supplied Go struct via the shared internal/scanner. Shape-
// identical to the DuckDB driver's Convertible — both are backed
// by *sql.DB.
...
func sqlRowsFromSource(ctx context.Context, source drivers.Source) (*sql.Rows, func(), error) {
	if source == nil {
		return nil, nil, fmt.Errorf("databricks: Convertible source is nil")
	}
	...
	if !ok {
		...
		return nil, nil, fmt.Errorf("databricks: Convertible source returned %T, want *sql.Rows", native)
	}
...
func collectSQLRows(rows *sql.Rows, out any) error {
	...
	if outVal.Kind() != reflect.Ptr || outVal.Elem().Kind() != reflect.Slice {
		return fmt.Errorf("databricks: out must be *[]T, got %T", out)
	}
	slice := outVal.Elem()
...
func elementTypeOf(sample any) (reflect.Type, error) {
	...
	if t == nil || t.Kind() != reflect.Ptr {
		return nil, fmt.Errorf("databricks: Stream sample must be *T, got %T", sample)
	}
	return t.Elem(), nil
}
